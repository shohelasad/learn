...
public class QuestionDAO implements PaginatableDAO {
    ...
    protected static final Integer FEATURED_SIZE = 25;
	...
	public List<Question> getFeaturedQuestions(String section) {
		DateTime since = DateTime.now().minusMonths(SINCE_MONTH);
		
		return top(section, FEATURED_SIZE, since);
	}
    
	public List<Question> getRecentQuestions(String channelName) {
		DateTime since = DateTime.now().minusMonths(SINCE_MONTH);

		return recentQuestions(channelName, FEATURED_SIZE, since);
	}
	...
	public List<Question> answered(Integer page) {
		Criteria criteria = session.createCriteria(Question.class, "q")
				.add(Restrictions.ne("q.answerCount", 0l))
				.addOrder(Order.desc("q.lastUpdatedAt"))
				.setMaxResults(PAGE_SIZE)
				.setFirstResult(firstResultOf(page))
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY);
	...
	public List<Question> recentQuestions(String channelName, int count, DateTime since) {
		Tag tag = tagDao.findByName(channelName);
		Criteria criteria;
		
		if(tag == null){
			criteria = session.createCriteria(Question.class, "q")
					//.createAlias("q.information.tags", "t")
					//.add(Restrictions.eq("t.id", tag.getId()))
					.addOrder(Order.desc("q.createdAt"))
					.setMaxResults(count)
	...
	public List<Question> recentUnanswered(String section, int count, DateTime since) {
		Order order = Order.desc("q.createdAt");
		
		return session.createCriteria(Question.class, "q")
				.add(and(Restrictions.eq("q.moderationOptions.invisible", false)))
				//.add(gt("q.createdAt", since))
				.add(and(isNull("q.solution"), Restrictions.gt("q.createdAt", since)))
				.addOrder(order)
				.setMaxResults(count)
				.list();
	}
