...
import static org.learn.model.EventType.ANSWERER_RELATED_EVENTS;
import static org.learn.model.EventType.ASKER_RELATED_EVENTS;
...
import org.learn.dto.KarmaByContextHistory;
import org.learn.dto.UserSummaryForTag;
...
public class ReputationEventDAO {
	...
	public KarmaByContextHistory karmaWonByQuestion(User user) {
		/*String hql = "select e.context.class, e.context.id, sum(e.karmaReward), e.date from ReputationEvent e " +
				"where e.user=:user " +
				"group by e.context.class, e.context.id, e.date " +
				"order by e.date desc";*/
	...
	private Query getTopAnswerersSummary(Tag tag, DateTime after) {
		...
		String hql = "select new org.learn.dto.UserSummaryForTag(sum(e.karmaReward) as karmaSum, count(distinct a), user) "
	...
	private Query getTopAskersSummary(Tag tag, DateTime after) {
		...
		String hql = "select new "
				+ "org.learn.dto.UserSummaryForTag(sum(e.karmaReward) as karmaSum, count(distinct q), user) "
				+ "from ReputationEvent e, Question q "+
				"join e.user user "+
				"join q.information.tags t "+
