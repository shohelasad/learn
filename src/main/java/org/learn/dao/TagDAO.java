...
public class TagDAO {
	...
	public List<TagUsage> getRecentTagsSince(DateTime since, int maxResult) {
		Query query = session.createQuery("select new org.learn.model.TagUsage(tag, count(question)) from Question question " +
	...
	public List<TagUsage> findMainTagsOfUser(User user) {
		Query query = session.createQuery("select new org.learn.model.TagUsage(tag, count(question)) from Answer answer " +
