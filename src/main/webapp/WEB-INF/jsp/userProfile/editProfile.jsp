...
<div class="subcontainer">
	...
	<%-- <div class="image-and-information1 edit-profile">
	    ...
	    <c:if test="${env.supports('feature.inhouse.upload')}">
		    <span>${t['user_profile.upload.photo']}</span><br/>
	        <span><input class="profile-photo-upload" data-user-id="${user.id}" type="file"></span>
	    </c:if>
	</div> --%>
	
	<div class="image-and-information profile-edit">
		<img class="profile-image" src="${user.getBigPhoto(env.get('gravatar.avatar.url'))}"/>
	    <c:if test="${!env.supports('feature.inhouse.upload')}">
		    <a href="${t['gravatar.url']}">${t['user_profile.edit.photo']}</a>
	    </c:if>
	...
	<div class="profile-edit-form">
		<form class="validated-form" action="${linkTo[UserProfileController].editProfile(user)}" method="POST">
			...
			<label for="occupation" class="form-label">${t['user_profile.edit.form.occupation.label']}</label>
			<input type="text" name="occupation" class="text-input" maxlength="100" value="<c:out value="${user.occupation}" escapeXml="true"/>"/>
			
			<label for="organization" class="form-label">${t['user_profile.edit.form.organization.label']}</label>
			<input type="text" name="organization" class="text-input" maxlength="100" value="<c:out value="${user.organization}" escapeXml="true"/>"/>
