...
<!-- <h2 class="title page-title subheader new-answer-title">আপনার উত্তর </h2> -->
...
<div class="summary question-summary item-title-wrapper">
	<form action="${uri}" method="post" class="validated-form1 form-with-upload ${not empty edit ? 'hinted-form' : '' } answer-form" data-same-author="${sameAuthor}">
		...
		<c:if test='${not empty edit}'>
			...
			<input type="text" data-hint-id="answer-comment-hint" placeholder="${t['edit_form.comment.placeholder']}" class="hintable required1 text-input" length="5" name="comment" />
		</c:if>
